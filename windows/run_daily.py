...
def run_batch(names, arguments, jitter=True):
    """Launch one scheduler process so accounts can rotate between problems."""
    log_dir = ROOT / 'logs'
    log_dir.mkdir(parents=True, exist_ok=True)
    with (log_dir / f'batch-{datetime.now():%Y-%m-%d}.log').open('a', encoding='utf-8', buffering=1) as output:
        record(output, f"START accounts={','.join(names)}")
        command = [
            sys.executable, '-u', str(ROOT / 'leetcode_bot.py'), '--no-menu',
            '--accounts', *names, '--batch-child', *arguments,
        ]
        if not jitter:
            command.append('--skip-start-jitter')
        process = None
        try:
            ...
            if process is not None and process.poll() is None:
                ...
                try:
                    ...
                    process.kill()
                    process.wait()
            record(output, 'Interrupted; inspect each profile before another run.')
            code = 130
        except Exception as error:
            record(output, f'LAUNCH ERROR: {error}')
            if process is not None and process.poll() is None:
            ...
            if process is not None and process.stdout is not None:
                process.stdout.close()
        record(output, f"END exit_code={code} accounts={','.join(names)}")
        return code
...
def main():
    ...
    parser.add_argument('--count', type=positive_count, default=None)
    ...
    try:
        ...
        if args.count is not None:
            arguments.extend(['--count', str(args.count)])
        arguments.extend(['--difficulty', ','.join(args.difficulty)])
        if args.instant:
